...
def process_request(data_str):
    try:
        ...
        print(f"[Service2.2] Processando {req_id} ...", flush=True)

        t_service_received = time.time()
        timestamps["service2_2_received"] = round(t_service_received, 6)
        ...
        timestamps["service2_2_sent"] = round(t_service_sent, 6)
        ...
        print(f"[Service2.2] Erro: {e}", flush=True)
        return json.dumps({
            "id": "ERROR",
            "timestamps": {},
            "error": str(e)
